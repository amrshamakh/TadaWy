...
export default function OfflineBookingReceiptModal({ receiptRef, receiptData, isPrinting, onPrintReceipt, onDone, isInline = false }) {
  const { t, i18n } = useTranslation();
  const isAr = i18n.language === "ar";

  if (!receiptData) return null;

  const { doctorName, patientName, patientEmail, specialty, phoneNumber, price, doctorLocation, doctorLocationDetails, receiptDate, date } = receiptData;
